...
void StaticServer::handleClient(socket_t clientSocket) {
    char buffer[4096] = {};
    ...
    std::filesystem::path fullPath = std::filesystem::path(rootDirectory) / path.substr(1); // Remove leading '/'
    ...
    try {
        ...
        std::cerr << "StaticServer: Error serving file " << path << ": " << e.what() << std::endl;
        send500(clientSocket);
    }
}
